...
class ServerlessPlugin {
    ...
    loadFixture(fixtureConfig) {
        ...
        if (fixtureConfig.sources && fixtureConfig.sources.length) {
            ...
            return Promise.all(fixtureConfig.sources.map(source => this.loadSource(
                source,
                fixtureConfig.table,
                this.dynamoFunctions.docWriteBatch,
                concurrentWrites,
                idAutogenerator,
            )));
        }
        ...
        if (fixtureConfig.rawsources && fixtureConfig.rawsources.length) {
            ...
            return Promise.all(fixtureConfig.rawsources.map(source => this.loadSource(
                source,
                fixtureConfig.table,
                this.dynamoFunctions.rawWriteBatch,
                concurrentWrites,
                idAutogenerator,
            )));
        }
    ...
    loadSource(sourceFilePath, tableName, dynamoFunction, concurrentWrites, autogeneratedId) {
        const fileFullPath = path.join(this.serverless.config.servicePath, sourceFilePath);
        if (!this.serverless.utils.fileExistsSync(fileFullPath)) {

            return Promise.reject(new Error(`${sourceFilePath} doesn't exist`));
        }

        const chunks = this.getSeedChunks(fileFullPath);
    ...
    getDynamoFunctions() {
        ...
        const raw = new DynamoDB(options);
        const doc = DynamoDBDocument.from(raw);

        return {
            docWriteBatch: (params) => doc.batchWrite(params),
            rawWriteBatch: (params) => raw.batchWriteItem(params),
        };
    }
    ...
    getAutogeneratedIdBlock(autogeneratedId) {
        ...
        if (autogeneratedId) {
            ...
            result.autogeneratedValueId = uuidv4();
        }
        return result;
    }
    ...
    getRawAutogeneratedIdBlock(autogeneratedId) {
        ...
        if (autogeneratedId) {
            result.autogeneratedKeyId = autogeneratedId;
            result.autogeneratedValueId = { S: uuidv4() };
        }
        return result;
    }
